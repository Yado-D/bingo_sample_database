...
@router.post("/transfer-credit", response_model=schemas.TransferResponse)
def transfer_credit(
    ...
    if sender_balance < float(payload.amount):
        raise HTTPException(status_code=402, detail="Insufficient balance")

    try:
        with db.begin():
            # lock sender and recipient rows for update
            sender = db.query(models.User).filter(models.User.id == current_user.id).with_for_update().one()
            recv = db.query(models.User).filter(models.User.id == recipient.id).with_for_update().one()

            # update balances (use wallet_balance)
            sender.wallet_balance = (sender.wallet_balance or 0.0) - float(payload.amount)
            recv.wallet_balance = (recv.wallet_balance or 0.0) + float(payload.amount)
            ...
            tx = models.PackageTransaction(
                ...
                package_amount=int(payload.amount) if isinstance(payload.amount, (int, float)) else payload.amount,
            )
            db.add(tx)
            db.flush()

            new_sender_balance = float(sender.wallet_balance)
            new_recipient_balance = float(recv.wallet_balance)

        return {"sender_balance": new_sender_balance, "recipient_balance": new_recipient_balance}
    except SQLAlchemyError as e:
        db.rollback()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Transfer failed: {e}")


@router.post("/package/revoke")
def revoke_package(
    payload: schemas.RevokeRequest,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(oauth2.get_current_user),
):
    # Fetch the package transaction
    tx = db.query(models.PackageTransaction).filter(models.PackageTransaction.id == payload.transaction_id).first()
    if not tx:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Transaction not found")

    # Verify current user is the sender
    if tx.sender_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only sender can revoke this transaction")

    # Fetch recipient
    recipient = db.query(models.User).filter(models.User.id == tx.receiver_id).first()
    if not recipient:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Recipient not found")

    amount = float(tx.package_amount or 0)

    # Check recipient has enough balance to reverse
    if float(getattr(recipient, 'wallet_balance', 0.0) or 0.0) < amount:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Insufficient funds on recipient to revoke")

    try:
        # Prefer starting a regular transaction; if the Session reports an
        # active transaction in a way that still causes `begin()` to fail,
        # fall back to a nested transaction (savepoint).
        from sqlalchemy.exc import InvalidRequestError

        try:
            txn = db.begin()
        except InvalidRequestError as ire:
            # If a transaction already exists on the Session, use a nested
            # transaction (savepoint) to execute this work safely.
            txn = db.begin_nested()

        with txn:
            # Lock sender and recipient rows
            sender = db.query(models.User).filter(models.User.id == tx.sender_id).with_for_update().one()
            recv = db.query(models.User).filter(models.User.id == tx.receiver_id).with_for_update().one()

            # Reverse balances (use wallet_balance)
            sender.wallet_balance = (sender.wallet_balance or 0.0) + amount
            recv.wallet_balance = (recv.wallet_balance or 0.0) - amount
        ...
        db.rollback()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Could not revoke transaction: {e}")
